...
if [[ "$DAI_VARIANT_SET" != "qualification" && "$DAI_VARIANT_SET" != "all" && "$DAI_VARIANT_SET" != "draft-profile" ]]; then
  echo "ERROR: DAI_VARIANT_SET must be qualification, all, or draft-profile." >&2
  exit 1
fi
...
jq -n --arg profile "$DAI_PROFILE" --arg region "$DAI_REGION" --arg run_id "$DAI_RUN_ID" \
  ...
  '{aws_profile:$profile,aws_region:$region,run_id:$run_id,expires_at_utc:$expires,
    ...
    instance_type:$instance_type,sglang_image:$sglang_image,sync_full_model:$sync_full_model}' > "$DAI_TFVARS"
